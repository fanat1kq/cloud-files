...
public class FileOperationsService {
          ...
          public ResourceInfoResponseDTO getResource(long userId, String path) {
                    try {
                              var resource = s3Repo.getResourceByPath(props.getDefaultBucketName(), paths.toTechnicalPath(userId, path));
          ...
          public void deleteResource(long userId, String path) {
                    ...
                    s3Repo.deleteResource(props.getDefaultBucketName(), paths.toTechnicalPath(userId, path));
          }

          public StreamingResponseBody download(long userId, String path) {
                    var resourceNames = findAllNames(userId, path, true);
          ...
          public ResourceInfoResponseDTO moveResource(long userId, String oldPath, String newPath) {
                    ...
                    return dtoMapper.toDto(userId, s3Repo.getResourceByPath(props.getDefaultBucketName(), newTech));
          }

          private StreamingResponseBody createSingleFileResponse(String resourceName) {
                    return os -> {
                              try (var is = s3Repo.getResourceByPath(props.getDefaultBucketName(), resourceName).dataStream()) {
                                        IOUtils.copy(is, os, props.getBufferSize());
                              }
                    };
          }
                    ...
                    return os -> {
                              try (var zos = new ZipOutputStream(os)) {
                                        resourceNames.forEach(name -> addToZip(userId,
                                                  s3Repo.getResourceByPath(props.getDefaultBucketName(), name), zos));
                              }
                    };
          }
                    ...
                    try {
                              ...
                              if (!paths.isDirectory(resource.path())) {
                                        try (var is = resource.dataStream()) {
                                                  IOUtils.copy(is, zos, props.getBufferSize());
                                        }
                              }
                              zos.closeEntry();
                    } catch (IOException e) {
          ...
          private void moveDir(String oldTech, String newTech) {
                    ...
                    List<String> sourcePaths = s3Repo.findAllNamesByPrefix(props.getDefaultBucketName(), oldPrefix, false);

                    sourcePaths.forEach(oldName -> {
                              String relativePath = oldName.substring(oldPrefix.length());
                              String newName = newPrefix + relativePath;

                              var resource = s3Repo.getResourceByPath(props.getDefaultBucketName(), oldName);
                              byte[] data;
                              try (InputStream is = resource.dataStream()) {
                                        data = is.readAllBytes();
                              } catch (IOException e) {
                                        throw new RuntimeException("Failed to read resource: " + oldName, e);
                              }
                              s3Repo.saveResource(props.getDefaultBucketName(), newName, new ByteArrayInputStream(data));
                    });

                    sourcePaths.forEach(oldName -> {
                              s3Repo.deleteResource(props.getDefaultBucketName(), oldName);
                    });
          }
          ...
          private void moveFile(String oldTech, String newTech) {
                    ...
                    s3Repo.saveResource(props.getDefaultBucketName(), newTech, resource.dataStream());
          }

          private void validateMove(long userId, String oldPath, String newPath) {
                    if (!resourceExists(userId, oldPath)) throw new ResourceNotFoundException(oldPath);
                    if (resourceExists(userId, newPath)) throw new ResourceAlreadyExistsException(newPath);
          }
