...
public class UploadService {
          ...
          private void validateUpload(long userId, String uploadPath, MultipartFile[] files) {
                    ...
                    Arrays.stream(files)
                              ...
                              .findFirst()
                              .ifPresent(file -> { throw new ResourceAlreadyExistsException(file.getOriginalFilename()); });
          }
          ...
          private void createDirs(long userId, String uploadPath, MultipartFile[] files) {
                    Arrays.stream(files)
                              ...
                              .forEach(dir -> s3Repo.createDirectory(props.getDefaultBucketName(), paths.toTechnicalPath(userId, dir)));
          }
          ...
          private Resource uploadFile(long userId, String uploadPath, MultipartFile file) {
                    ...
                    try (var is = file.getInputStream()) {
                              s3Repo.saveResource(props.getDefaultBucketName(), techPath, is);
                              return s3Repo.getResourceByPath(props.getDefaultBucketName(), techPath);
                    } catch (Exception e) {
